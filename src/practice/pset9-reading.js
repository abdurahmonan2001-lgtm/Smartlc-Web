...
The obvious explanation, that we simply forget where we were, is only a small part of the story. The deeper mechanism has been given a name: attention residue. When we move from one task to another, a portion of our attention stays behind, still processing the unfinished business of the first task. The residue is heaviest when the abandoned job was left in an ambiguous state, with no clear plan for finishing it, which is precisely the state that most interruptions create. The effect has been reproduced in the laboratory, where volunteers moved off a puzzle before they had solved it went on thinking about it during the task that followed. The person who turns back to a report after a five-minute conversation is therefore not quite the worker who left it; for some minutes they are running on reduced capacity, and they are generally unaware of the fact. Self-assessment in this area is poor. Asked afterwards, interrupted workers tend to report that they lost a moment or two.
...
Two cautions are worth keeping in view. Interruption is not simply a harm to be minimised. The question from the next desk often prevents a week of work in the wrong direction, and organisations that make themselves uninterruptible tend to find that nobody is checking anybody. The error caught in a corridor is the cheapest error there is. What is wrong is not that interruption exists but that its price and its distribution are currently set by accident. Nor will any single fix do the job: switching off notifications while the surrounding culture still expects instant answers merely moves the pressure somewhere else. The cost of an interruption is real, measurable and largely invisible, and it is paid, in the end, by whoever is least able to defend their attention.`,
      questions: [
